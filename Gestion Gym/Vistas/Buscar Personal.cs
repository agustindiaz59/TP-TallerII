...
namespace Gestion_Gym
{
    public partial class Buscar_Personal : Form
    {
        ...
        public void CargarPersonal(List<Personal> datos)//string filtro = "", int pageNumber = 1, int pageSize = 20)
        {
            dataGridView1.Columns.Clear();
            dataGridView1.DataSource = datos;
            AjustarDataGridView();
        ...
        {
            CargarPersonal(PersonalDAO.TraerTodos());  // Carga todos los miembros al abrir el formulario
            bsnombre.Focus();
        }
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
           try
           {
               string CUIL = dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString();
               //Abrir formulario de editar
               Editar_Personal editar = new Editar_Personal(CUIL,this);
               editar.Show(this);

            }
           catch (Exception ex) 
           {
               MessageBox.Show("Error buscando el Personal");
           }
        ...
        {
            CargarPersonal(PersonalDAO.buscarPorCuil(textBox2.Text));
        }

        private void bsnombre_TextChanged(object sender, EventArgs e)
        {
            CargarPersonal(PersonalDAO.buscarPorNombre(bsnombre.Text));

        }

        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            CargarPersonal(PersonalDAO.buscarPorApellido(textBox1.Text));
        ...
        {
            CargarPersonal(PersonalDAO.TraerTodos());
        }
